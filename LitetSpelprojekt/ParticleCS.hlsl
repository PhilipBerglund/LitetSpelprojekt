...
void CSMAIN(uint3 DispatchThreadID : SV_DispatchThreadID)
{
    ...
    unsigned int myID = DispatchThreadID.x
    ...
    if (myID < NumParticles.x)
    {
        ...
        // Calculate the current graviattional force applied to it
        float3 d = ConsumerLocation.xyz - p.position;
        float r = length(d);
        float3 Force = (G * m1m2 / (r * r)) * normalize(d);
        
        // Calculate the new velocity, accounting for the acceleration
        // from the gravitational force over the current time stop.
